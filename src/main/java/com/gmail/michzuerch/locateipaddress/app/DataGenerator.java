...
public class DataGenerator implements HasLogger {
    ...
    private void createLocationsAndBlocks(LocationRepository locationRepository, BlockRepository blockRepository) {
        Location location = createLocation("city", "cityName", "continentCode", "continentName", "country",
                "countryIsoCode", "countryName", "geonameId", "isInEuropeanUnion", new BigDecimal(0), "localeCode",
                "metroCode", "timeZone", new BigDecimal(1), "postalcode", "subdivision1isoCode", "subdivision1name",
                "subdivision2isoCode", "subdivision2name");

        Block block1 = createBlock("accuracyRadius", new BigDecimal(0), "geonameId", "isAnonymousProxy",
                "isSatelliteProvider", "234234", "62345", "network", "postalCode", "registeredCountryGeonameId",
                "representedCountryGeonameId", new BigDecimal("4"));

        Block block2 = createBlock("accuracyRadius", new BigDecimal(0), "geonameId", "isAnonymousProxy",
                "isSatelliteProvider", "234234", "62345", "network", "postalCode", "registeredCountryGeonameId",
                "representedCountryGeonameId", new BigDecimal("4"));

        Block block3 = createBlock("accuracyRadius", new BigDecimal(0), "geonameId", "isAnonymousProxy",
                "isSatelliteProvider", "234234", "62345", "network", "postalCode", "registeredCountryGeonameId",
                "representedCountryGeonameId", new BigDecimal("4"));

        location = locationRepository.save(location);

        block1.setLocation(location);
        block1 = blockRepository.save(block1);

        block2.setLocation(location);
        block2 = blockRepository.save(block2);

        block3.setLocation(location);
    ...
    private Block createBlock(String accuracyRadius, BigDecimal endip, String geonameId,
        ...
        block.setIsAnonymousProxy(isAnonymousProxy);
        block.setIsSatelliteProvider(isSatelliteProvider);
        block.setLatitude(latitude);
        block.setLongitude(longitude);
        block.setNetwork(network);
        block.setPostalCode(postalCode);
        block.setRegisteredCountryGeonameId(registeredCountryGeonameId);
        block.setRepresentedCountryGeonameId(representedCountryGeonameId);
        block.setStartip(startip);
        return block;
    }
    ...
    private Location createLocation(String city, String cityName, String continentCode, String continentName,
        ...
        location.setContinentCode(continentCode);
        location.setContinentName(continentName);
        location.setCountry(country);
        location.setCountryIsoCode(countryIsoCode);
        location.setCountryName(countryName);
        location.setGeonameId(geonameId);
        location.setIsInEuropeanUnion(isInEuropeanUnion);
        location.setLatitude(latitude);
        location.setLocaleCode(localeCode);
        location.setMetroCode(metroCode);
        location.setTimeZone(timeZone);
        location.setLongitude(longitude);
        location.setPostalcode(postalcode);
        location.setSubdivision1isoCode(subdivision1isoCode);
        location.setSubdivision1name(subdivision1name);
        location.setSubdivision2isoCode(subdivision2isoCode);
        location.setSubdivision2name(subdivision2name);
        return location;
    }
}
