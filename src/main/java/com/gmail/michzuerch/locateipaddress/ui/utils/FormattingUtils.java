...
public class FormattingUtils {
	...
	public static final DateTimeFormatter MONTH_AND_DAY_FORMATTER = DateTimeFormatter.ofPattern("MMM d",
			LocateIPAddressConst.APP_LOCALE);
	 ...
	 */
	public static final TemporalField WEEK_OF_YEAR_FIELD = WeekFields.of(LocateIPAddressConst.APP_LOCALE).weekOfWeekBasedYear();
	...
	public static String getFullMonthName(LocalDate date) {
		return date.getMonth().getDisplayName(TextStyle.FULL, LocateIPAddressConst.APP_LOCALE);
	}

	public static String formatAsCurrency(int valueInCents) {
		return NumberFormat.getCurrencyInstance(LocateIPAddressConst.APP_LOCALE).format(BigDecimal.valueOf(valueInCents, 2));
	}

	public static DecimalFormat getUiPriceFormatter() {
		DecimalFormat formatter = new DecimalFormat("#" + DECIMAL_ZERO,
				DecimalFormatSymbols.getInstance(LocateIPAddressConst.APP_LOCALE));
